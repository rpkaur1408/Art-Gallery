...
export default function MainNav() {
  ...
  const handleSearchSubmit = async (e) => { // Make function asynchronous
    e.preventDefault();
    const query = `title=true&q=${encodeURIComponent(searchField)}`;
    setSearchHistory(await addToHistory(query)); // Update history using addToHistory
    router.push(`/artwork?${query}`); // Redirect to the artwork page
    setIsExpanded(false);
  };
